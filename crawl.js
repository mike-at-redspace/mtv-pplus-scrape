...
const scrapeGoogleSearchResults = async () => {
    try {
        ...
        while (await hasMoreResults(page) || await hasOmittedResults(page)) {
            try {
                const moreResultsButton = await page.waitForSelector('a[aria-label="More results"]', { timeout: 1000 }).catch(() => null);
                const omittedResultsButton = await page.waitForSelector('a:has-text("omitted")', { timeout: 1000 }).catch(() => null);
        ...
        console.log(chalk.greenBright(`Google Search Results saved to: ${outputFile}`));
        await browser.close();
    } catch (error) {
